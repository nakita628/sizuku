...
describe("parseAnnotations", () => {
  it("parses zod annotation", () => {
    ...
    expect(result).toStrictEqual([{ type: "zod", key: "schema", value: "string().min(1)" }]);
  });
  ...
  it("parses arktype annotation", () => {
    ...
    expect(result).toStrictEqual([{ type: "arktype", key: "schema", value: '"string.uuid"' }]);
  });

  it("parses effect annotation", () => {
    const result = parseAnnotations("@e.Schema.UUID");
    expect(result).toStrictEqual([{ type: "effect", key: "schema", value: "Schema.UUID" }]);
  });
...
export const user = pgTable('user', {
  ...
  name: text('name').notNull(),
    ...
    expect(tableAnnotations).toStrictEqual([{ type: "zod", key: "schema", value: "strictObject" }]);
...
export const post = pgTable('post', {
  ...
  userId: uuid('user_id').notNull(),
    ...
    expect(userIdAnnotations).toStrictEqual([{ type: "zod", key: "schema", value: "uuid()" }]);

    const postIdAnnotations = result.columnComments.get("post.id");
    expect(postIdAnnotations).toStrictEqual([{ type: "zod", key: "schema", value: "uuid()" }]);

    const postUserIdAnnotations = result.columnComments.get("post.userId");
    expect(postUserIdAnnotations).toStrictEqual([{ type: "zod", key: "schema", value: "uuid()" }]);
  });
});
