...
describe('sizuku argv parsing', () => {
  ...
  it('returns error for unsupported output format .txt', async () => {
    ...
    expect(result.ok).toBe(false)
    if (!result.ok) {
      expect(result.error).toBe('Unsupported output format: output.txt. Supported: .dbml, .png, .md')
    }
  })
  ...
  it('returns error for unsupported output format .ts', async () => {
    ...
    expect(result.ok).toBe(false)
    if (!result.ok) {
      expect(result.error).toBe('Unsupported output format: output.ts. Supported: .dbml, .png, .md')
    }
  })
...
describe('E2E: DBML generation', () => {
  ...
  it('generates DBML with correct table and column definitions for EC schema', async () => {
    ...
    expect(content.includes('Ref orderItem_orderId_order_id_fk: orderItem.orderId > order.id')).toBe(true)
  })
